...
:::{.chapterintro}
Bij de analyse van data probeer je antwoorden te vingen op vragen door statistische methodes tot te passen op waarnemingen. Deze waarnemingen - verzameld uit rapportages, enquêtes en experimenten - vormen de ruggegraat van een statistisch onderzoek en worden **data** genoemd. Wanneer je deze data uit externe bronnen importeert, heb je geen controle over de indeling en het type gegevens en de manier waarop de data is georganiseerd. Voordat je met de analyse kunt beginnen moet je daarom vaak veel tijd steken aan het opschonen en herstructureren van de gegevens.
In dit hoofdstuk ligt de nadruk op de eigenschappen van data, variabelen en waarnemingen, de manier waarop deze vaak worden aangeboden en hoe je deze in een gewenste vorm kunt brengen.
:::

Veel gegevensbestanden zijn vervuild, gegevens ontbreken of de gegevens zijn niet goed gestructureerd. Excel heeft enkele functies om je daarbij te helpen. Maar helaas kan Excel jouw handelingen niet vastleggen. Daarom is het belangrijk om te documenteren wat je hebt gedaan met de oorspronkelijke gegevens om deze geschikt te maken voor analyse. Acties die je met de Power Query-Editor uitvoert worden in een script vastgelegd waardoor de uitgevoerde acties reproduceerbaar zijn. Probeer daarom altijd zoveel mogelijk de Power-Query-Editor te gebruiken voor het opschonen en herstructureren van de data.

Wanneer de gegevens goed gestructureerd zijn kun je ze gemakkelijker modelleren, visualiseren en transformeren waardoor de analyse eenvoudiger wordt.
...
knitr::kable(exp1, caption = 'Personen als rij.')
```
...
knitr::kable(exp2, caption = 'Behandelingen als rij.')
```
...
knitr::kable(exp3, caption = 'Gegevens in gestructureerde vorm.')
```
...
scores1 %>% kbl(caption = "Tabel waarin de kolomkoppen man en vrouw in feite waarden van variabele geslacht zijn.") %>% kable_styling()
```
...
```{r scores1-result, out.width="40%", fig.cap="Scores1 nu gestructureerd."}
knitr::include_graphics("images/data/scores1_result.png")
```
...
scores2 %>% kbl(caption = "Tabel met vier kolomkoppen die een combinatie van de variabelen geslacht en klas zijn.") %>% kable_styling()
```
...
```{r scores2-result, out.width="40%", fig.cap="Scores2 nu gestructureerd."}
knitr::include_graphics("images/data/scores2_result.png")
```
...
scores3 %>% kbl(caption = "Een tabel met variabelen in zowel rijen als kolommen.") %>% kable_styling()
```
...
```{r scores3-result, out.width="40%", fig.cap="Scores3 nu gestructureerd."}
knitr::include_graphics("images/data/scores3_result.png")
```
...
Benelux_Populatie %>% kbl(caption = "Populatie Benelux voor de jaren 2000, 2005, 2010 en 2015.") %>% kable_styling()
```
...
1. Selecteer in de Power Query editor de eerste twee kolommen, met `Land` en `Geslacht`.
...
1. Kies *tab Startpagina >Sluiten en laden (groep Sluiten)*.
...
## Meetniveaus variabelen {#meetniveaus}
...
+ **Continue variabelen**: numerieke variabelen die elke waarde kunnen aannemen. Dit is meestal het geval bij het uitvoeren van metingen. Bijvoorbeeld de lengte van een persoon.

**Afhankelijke(verklaarde) en onafhankelijke (verklarende) variabelen**

In een onderzoek wordt vaak geprobeerd om met de ene variabele een verklaring te vinden of een voorspelling te doen voor een andere variabele.

$Onafhankelijke (verklarende) variabele \rightarrow Afhankelijke (verklaarde) variabele$


## Opgaven {#opg-tidydata}
...
tidy001.data %>% kbl(format.args = list(decimal.mark = ","), caption = "Starbucks koffieprijzen voor drie verschillende groottes.") %>% kable_styling(full_width = FALSE)
```

4. Om de kosten van levensonderhoud in Europa te vergelijken zijn via de website van [Numbeo](https://www.numbeo.com/cost-of-living/) voor een aantal plaatsen de gemiddelde marktprijzen verzameld voor brood (wit, 500 gram), kaas(lokaal, 1 kg), melk (gewoon, 1 liter) en rijst (wit, 1 kg). De data staan in het hulpbestand [levensonderhoud.xlsx.xlsx](data/levensonderhoud.xlsx). Voor de eerste 8 plaatsen zijn de gegevens te zien in tabel \@ref(tab:levensonderhoud-data).
...
levensonderhoud.data %>% head(8) %>% kbl(format.args = list(decimal.mark = ","), caption = "Gemiddelde prijzen voor vier produkten.") %>% kable_styling(full_width = FALSE)
```
...
rekenentaal.data %>% kbl(caption = "Schoolresultaten voor rekenen en taal.") %>% kable_styling(full_width = FALSE)
```
